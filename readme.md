...
You may also have to update the *Content-Security-Policy* header to allow scripts from your server.
...
* many old tutorials recommend using `var x = whatever;`.
  **never use `var`, always use `let` (as in `let x = whatever;`).**
  `var` has has insane scoping rules, e.g. you can use the variable before declaring it and after its scope has expired.
...
See docs for [addEventListener](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener) and the [events](https://developer.mozilla.org/en-US/docs/Web/Events).
...
* create a new javascript file in src/main/resources/public.
  include the file in the forum thread view html.
...
* write a javascript function that has one parameter: the forum thread's id.
  the function should do a `XMLHttpRequest` to the new request hander and send the forum thread's id.
...
* send the id as a *request parameter*.
  ...
  the parameters can be accessed by adding the `@RequestParam Long id` parameter to the request hander.
...
The parameters can be optional, but the request uri still stays the same.
...
* add a element to the html thread view for showing the currently writing users for the open thread.
  create the event source in javascript which receives events from the server and updates the list of writing users in the html element.
